...
Generally, the separatrix splitting positive from negative samples in an $n$-dimensional space $\mathds{R}^n$ is an $n-1$-dimensional subspace (called \textit{hyperplane}), which in the case of $\mathds{R}^3$ corresponds to a plane. 
%https://en.wikipedia.org/wiki/Plane_(geometry)#Representation

The general form of the equation of a plane is given as the following linear equation:
...
This reads as "All points $(x,y,z)$ for which \ref{eq:general_plane} holds are on the plane". Further, the vector $(a,b,c)$ encodes an normal vector orthogonal to this plane, whereas d serves as intercept, encoding the intersection of plane and normal. In higher dimensions, the formula for the hyperplane would become $a_1x_1+a_2x_2+a_3x_3+...+a_nx_n = b$, which means that encoding the hyperplane for a space $\mathds{R}^n$ requires $n+1$ parameters.
...
The distance from any point of this $\mathds{R}^n$ to the hyperplane is then calculated as the length of the vector that is the \textit{orthogonal projection} from the point onto the hyperplane. The orthogonal projection from one vector onto another can be calculated as 
