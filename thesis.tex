...
\ifdefined\ForPrinting
	...
	\ohead{\headmark}% Inner header
	\ihead[\pagemark]{\pagemark}% Outer header
\else
...
\let\origsection\section % Backup old section-command BEFORE placeins https://tex.stackexchange.com/a/303448/108199
...
\newacronym{iou}{IoU}{Intercept over Union}
...
\ifdefined\ForPrinting
	...
	\newcommand{\backref}[1]{\ignorespaces}	
	\newcommand{\hideref}[2]{\ignorespaces}	
	\newcommand{\arrowback}{\ignorespaces}	
\else
	...
	\newcommand{\backref}[1]{\texorpdfstring{\hyperref[#1]{{\scriptsize\xspace back}}}{}}
	\newcommand{\hideref}[2]{\texorpdfstring{\hyperref[#1]{{\scriptsize\xspace #2}}}{}}
	\newcommand{\arrowback}{\Acrobatmenu{GoBack}{\textsuperscript{\textuparrow}}}
\fi
...
\setlist{nolistsep} % such that pandoc nested lists are closer
\setlist{nosep}     % ^
...
\definecolor{evenmorelightgray2}{gray}{0.96}
...
\ifdefined\RemoveAll
	\newcommand{\removeMe}[1]{\ignorespaces}
\else
	\newcommand{\removeMe}[1]{#1}
\fi
...
\newcommand{\todo}{\todoparagraph{-----TODO-----}}
...
\newcommand{\acc}[2]{{\scriptsize (accessed at \date{2022}{#1}{#2})}}

% https://tex.stackexchange.com/a/524575/108199
...
\def\urlx #1{\href{#1}{\hyphenateAndTtWholeString{#1}}} % Define the \urlx command which works like \url, but with line breaks


\ifdefined\ForPrinting
	\let\oldcaption\caption
\else
	\addto\captionsenglish{\renewcommand{\figurename}{Figure{\tiny \Acrobatmenu{GoBack}{\textsuperscript{\textuparrow}}}\hspace{-0.8ex}}}
	\addto\captionsbritish{\renewcommand{\figurename}{Figure{\tiny \Acrobatmenu{GoBack}{\textsuperscript{\textuparrow}}}\hspace{-0.8ex}}}
	\addto\captionsenglish{\renewcommand{\tablename}{Table{\tiny \Acrobatmenu{GoBack}{\textsuperscript{\textuparrow}}}\hspace{-0.8ex}}}
	\addto\captionsbritish{\renewcommand{\tablename}{Table{\tiny \Acrobatmenu{GoBack}{\textsuperscript{\textuparrow}}}\hspace{-0.8ex}}}
\fi
...
\usepackage[final]{pdfpages} % very last thing, option clash with graphicx
...
\newgeometry{bottom=15mm, top=30mm, inner=26mm, outer=26mm}
...
\ifdefined\ForPrinting
	\newgeometry{a4paper,top=0mm,bottom=0mm,inner=0mm,outer=0mm}
	\includegraphics[width=\textwidth]{declaration_pretty.pdf}
	\restoregeometry
	% don't effing ask why just having this as a chapter doesn't work in twopage.
\else
...
\begin{abstract}
	\ifdefined\ForPrinting\clearpage\fi
...
\newgeometry{bottom=15mm, top=30mm, inner=22mm, outer=30mm}
...
\printglossary[type=\acronymtype, title=Acronyms,    toctitle=List of Acronyms, style=long3colheaderMINE2] %TODO: If it becomes more, add `nogroupskip `'
