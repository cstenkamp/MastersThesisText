...
\todoparagraph{SHORTLY mention if my results cannot confirm stuff from derrac and refert to discussion}
...
\todoparagraph{Say that more details are in the appendix}
\todoparagraph{Keep in mind that the architecture is very modular and there are alternatives one could do at every step, but hyperparams and also which components to use is a cominbatiorical explosion}
...
\todoparagraph{In contrast to the previous explanations here I am directly talking about parameter (choice)s at each step etc}
\todoparagraph{In contrast the the previous explanations here I am talking about how III did it}
\todoparagraph{Also we will come back to this in future work were we list other stuff that one could do for every step}
...
\todoparagraph{Schon hier dass alshaikh kein mds nutzen oder dass man ja nicht fur jeden classifier einzeln die svm machen muss}
...
\todoparagraph{Mep, the fact that mainalgos explicitly did it per-dataset, ref table}
...
\todoparagraph{LEMMATIZATION LINK:} % https://textmining.wp.hs-hannover.de/Preprocessing.html#Lemmatisierung
...
\todoparagraph{Ensure this is consistent with later parts where I say the number of candidates: thing is I have less words but the algorithm seems to profit from less words as that makes it more robust}

\todoparagraph{Come back to this in optimal parameters section }

\todoparagraph{Just using all words would have been a possibility as well, but than we would not have ngrams.}
...
\todoparagraph{The shitton of work that keybert was argh}
...
\todoparagraph{grains of salt with vectors vs points}
